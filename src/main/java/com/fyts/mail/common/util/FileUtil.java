...
public class FileUtil extends cn.hutool.core.io.FileUtil {
	...
	private static final int compressWidth = 50;
	
	private static final int compressHeight = 50;
	
	private static final String compressDir = "min";
	
	private static final String originalDir = "org";
	...
	public static String uploadFile(MultipartFile file, String dirName){
		...
		String filePath = "/"+dirName+"/" + DateUtil.format(date,"yyyyMMdd") +  "/" + DateUtil.format(date,"yyyyMMddHHmmss") + RandomUtil.randomString(5)+"."+ extName(file.getOriginalFilename());
		...
		try {
			...
			if (!outFile.getParentFile().exists()) {
				...
				if (!outFile.getParentFile().mkdirs()) {
					throw new RuntimeException("创建目标文件所在的目录失败!");
				}
			}
			...
			if(file.getContentType().contains("image")) {
				...
				if (!targerFile.getParentFile().exists()) {
					...
					// 创建目标文件所在的目录
					if (!targerFile.getParentFile().mkdirs()) {
						throw new RuntimeException("创建缩略图文件所在的目录失败!");
					}
				}
				ImgUtil.scale(outFile, targerFile, compressWidth, compressHeight, null);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String downFile(File file, HttpServletRequest request, HttpServletResponse response, String fileName){
		...
		try {
			response.addHeader("Content-Disposition", "attachment; filename=\"" +
					URLUtil.encode(StrUtil.isBlank(fileName) ? file.getName() : fileName) + "\"");
	...
	public static String path(String path){
		...
		p = StrUtil.join("/", StrUtil.split(p, "/"));
